warn("[SDA Pro] Setting up support functions")
local cloneref = cloneref or function(x) return x end
local function getscriptdirectory(scr)
    local result = scr.Name
    scr = scr.Parent
    while scr and scr ~= game do
        result = scr.Name.."/"..result
        scr = scr.Parent
        task.wait()
    end
    return "game/"..result
...
function recTable(Table)
    local TableString = "\n{"

    for i,v in pairs(Table) do
        if typeof(v) == "function" then 
            TableString = TableString.."\n # FUNC "..tostring(v).." #\n"
            for fI, fV in pairs(debug.getconstants(v)) do
                TableString = TableString.."["..tostring(fI).."] "..tostring(fV).."\n"
            end
            TableString = TableString.."\n # FUNC "..tostring(v).." END #\n"
        end
        if typeof(v) == "table" then
            TableString = TableString..recTable(v).."\n"
        else
            TableString = TableString.." ["..tostring(i).."] "..tostring(v).." \n"
        end
    end
    TableString = TableString.."}\n"
    return TableString
...
local dumpstring = getscriptbytecode or dumpstring or nil
local getscripthash = getscripthash or nil
local runningscripts = getrunningscripts or nil
local isfolder = isfolder or nil
local makefolder = makefolder or nil
local writefile = writefile or nil
local senv = getsenv or nil

warn("[SDA Pro] Getting Services")
local HttpService = cloneref(game:GetService("HttpService"))

warn("[SDA Pro] setting up enviroment")

if not isfolder("SDAPro") then
    makefolder("SDAPro")
end


if not isfolder("SDAPro/Decompiled") then
    makefolder("SDAPro/Decompiled")
end 

if not isfolder("SDAPro/Dumps") then
    makefolder("SDAPro/Dumps")
end

if not isfolder("SDAPro/Dumps/"..game.Name) then
    makefolder("SDAPro/Dumps/"..game.Name)
end

if not isfolder("SDAPro/Decompiled/"..game.Name) then
    makefolder("SDAPro/Decompiled/"..game.Name)
end 

local DumpUUID = HttpService:GenerateGUID(false)

if not isfolder("SDAPro/Dumps/"..game.Name.."/"..DumpUUID) then
    makefolder("SDAPro/Dumps/"..game.Name.."/"..DumpUUID)
end

if not isfolder("SDAPro/Decompiled/"..game.Name.."/"..DumpUUID) then
    makefolder("SDAPro/Decompiled/"..game.Name.."/"..DumpUUID)
end 

local function emulatescriptdirectory(Directory, dumpString, curMap)

    local folders = string.split(Directory, "/")

    for i,v in pairs(folders) do
        if i == #folders then
            writefile(curMap.."/"..v..".lua", dumpString)
            continue
        end
        if not isfolder(curMap.."/"..v) then
            makefolder(curMap.."/"..v)
            curMap = curMap.."/"..v
        end
    end
...
warn("[SDA Pro] Getting game infomation...")

local gameData = {}
gameData.PlaceId = game.PlaceId
gameData.ServerDump = game.JobId
gameData.TotalScripts = 0
gameData.ModuleScripts = 0
gameData.LocalScripts = 0
gameData.ScriptHash = ""

warn("[SDA Pro] Finding scripts...")

local Completed = 0
local Errored = 0
local CurIndex = 0
local GameScripts = runningscripts()

warn("[SDA Pro] Found scripts... Dumping... ("..(Completed + Errored).."/"..#GameScripts.." ("..Errored.." has errored.)")

for index, scr in pairs(GameScripts) do
    gameData.TotalScripts += 1
    if scr:IsA("LocalScript") then
        gameData.LocalScripts += 1
    else
        gameData.ModuleScripts += 1
    end
end
...
for index, scr in pairs(GameScripts) do
    ...
    CurIndex += 1;
    if CurIndex == 1 then
        gameData.ScriptHash = getscripthash(scr)
    end
    ...
    local s,e = pcall(function()
        ...
        DumpString = DumpString.."Script Hash: "..getscripthash(scr).."\n"
        DumpString = DumpString.."Script Directory: "..getscriptdirectory(scr).."\n"
        DumpString = DumpString.."Script Class: "..scr.ClassName.."\n"
        DumpString = DumpString.."Script Name: "..scr.Name.."\n"
    
        local ScriptEnv = senv(scr)
    
        DumpString = DumpString.."### SCRIPT ENVIROMENT ###\n"
    
        for eI, eV in pairs(ScriptEnv) do
            if typeof(eV) == "table" then
                DumpString = DumpString.."["..eI.."]"..recTable(eV).."\n"
            elseif typeof(eV) == "function" then
                DumpString = DumpString.."\n # FUNC "..tostring(eV).." #\n"
                for fI, fV in pairs(debug.getconstants(eV)) do
                    DumpString = DumpString.."["..tostring(fI).."] "..tostring(fV).."\n"
                end
                DumpString = DumpString.."\n # FUNC "..tostring(eV).." END #\n"
            else
                
                DumpString = DumpString.."["..tostring(eI).."]"..tostring(eV).."\n"
            end
        end

        DumpString = DumpString.."### BYTE CODE ###\n" 
        DumpString = DumpString..tostring(getscriptbytecode(scr)).."\n"

        emulatescriptdirectory(getscriptdirectory(scr), DumpString, "SDAPro/Dumps/"..game.Name.."/"..DumpUUID)
...
warn("[SDA Pro] Dumped!")
warn("[SDA Pro] ("..(Completed + Errored).."/"..#GameScripts..")")
warn("[SDA Pro] Successful dumped "..(Completed).." / "..(Errored).." | Success rate of: "..((Completed / #GameScripts) * 100).."%")

warn("[SDA Decompiler] Starting Disassembler")
...
for i,v in pairs(GameScripts) do
    warn("[SDA Pro] Dissambling script: "..CurIndex.."/"..#GameScripts.." | "..((CurIndex / #GameScripts) * 100).."%")

    local s,e = pcall(function()
        local decomped = _ENV.decompile(v, { 
            DecompilerTimeout = 5,
            DecompilerMode = "disasm",
        ...
        emulatescriptdirectory(getscriptdirectory(v), decomped, "SDAPro/Decompiled/"..game.Name.."/"..DumpUUID)

        Completed += 1
        warn("[SDA Pro] Dissambled "..getscriptdirectory(v).." | "..CurIndex.."/"..#GameScripts.." | "..((CurIndex / #GameScripts) * 100).."%")
        CurIndex += 1
    end)
    if e then
        Errored += 1
...
warn("[SDA Pro] All Scripts dissambled!")
warn("[SDA Pro] "..Errored.." script has failed! ("..((Errored/Completed)*100).."%)")
